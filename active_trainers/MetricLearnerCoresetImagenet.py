...
# from arch.wide_resnet import WideResNet_metric, conv_init
from data_utils import imagenet64

from arch.resnetv1 import ResNet50_metric
from arch.wide_resnet import WideResNet_metric
...
class MetricTrainer(BaseActiveTrainer):
    def __init__(self, config, device=torch.device('cuda:0'), logger=None, positive_margin=0.3,start_epoch=5, distance=1.):
        self.config = config
        self.device = device
        self.logger = logger
        self.module_lr = 0.01
    ...
    def start_loop(self, net, train_dataset, test_dataset, unlabel_dataset, n_iteration, index_loop=1, is_semi=False, is_semi_metric=False, pesudo_count=1000, semi_ind=None, is_mixup=True, is_rubost=False):
        method_str = '====================Metric Method with pesudo label==============='
        self.logger.info(method_str)
        self.n_select = self.config['n_select']
        self.n_initial = self.config['n_initial']
        # self.n_initial = 200
        self.loop_num = index_loop
        self.pesudo_count=pesudo_count
        total_train_ind = np.arange(self.config['n_total'])
        self.semi_ind = semi_ind
        self.label_ind = np.random.permutation(total_train_ind)[:self.n_initial]
        ...
        size = 64
        config = [self.config['depth'], self.config['k'], self.config['drop'], self.config['n_cls'], size]
        for i in range(n_iteration):
            param_path = './chechkpoints/param_loop_%d_iteration_%d.pt' % (self.loop_num, i)
            metric_center_path = './chechkpoints/center_loop_%d_iteration_%d.npy' % (self.loop_num, i)
            label_ind_path = 'label_ind_loop_%d_iteration_%d.npy' % (self.loop_num , i)
            unlabel_ind_path = 'unlabel_ind_loop_%d_iteration_%d.npy' % (self.loop_num , i)

            # if os.path.exists(param_path) and os.path.exists(label_ind_path):
            #     print('load param loop %d iteration %d' % (self.loop_num, i))
            #     self.label_ind = np.load(label_ind_path)
            #     self.unlabel_ind = np.load(unlabel_ind_path)



            import os
            if os.path.exists(label_ind_path) and os.path.exists(unlabel_ind_path):
                self.label_ind = np.load(label_ind_path)
                self.unlabel_ind = np.load(unlabel_ind_path)
                continue
            ...
            str = '[Iteration %d] select index:' % self.loop_num
            ...
            #                                         ind_u=self.unlabel_ind)
                ...
                # model.apply(self.init_weights)
            # model = ResNet50_metric(self.config['n_cls']).to(self.device)
            model = WideResNet_metric(config).to(self.device)
            ...
            acc_info = '[Iteration %d]epoch %d Accuracy %.4f metric acc %.4f' % (self.loop_num, i, acc, metric_acc)
            print(acc_info)
            self.logger.info(acc_info)
            ...
            ds_u = imagenet64.get_unlabel_dataset()
            ...
            query_ind = self.get_query_idx(model, dl_u, self.n_select)
            end = time.time() - aa
            ...
            self.unlabel_ind = np.setdiff1d(total_train_ind, self.label_ind)
            np.save(label_ind_path, self.label_ind)
            np.save(unlabel_ind_path, self.unlabel_ind)


    def train_pesudo_data(self, model):

        ds_x = imagenet64.get_train_dataset()
        print(len(self.label_ind))
        ...
        ds_g = imagenet64.get_train_dataset()
        ...
        for epoch in range(self.config['n_epoch']):
            # param_path = './chechkpoints/param_iteration_%d_epoch_%d.pt' % (self.index_iteration, epoch)
            # metric_center_path = './chechkpoints/center_iteration_%d_epoch_%d.npy' % (self.index_iteration, epoch)
            # if os.path.exists(param_path) and os.path.exists(metric_center_path):
            #     print('load param epoch %d' % epoch)
            #     model.load_state_dict(torch.load(param_path))
            #     self.mean_embedding = np.load(metric_center_path)
            #     sched_backbone.step()
            ...
            self.train_pesudo_each_epoch(model, optim_backbone, criterions, dl_x, dl_g, epoch)
                ...
                # np.save(metric_center_path, self.mean_embedding)
            sched_backbone.step()
    ...
    def train_pesudo_each_epoch(self, model, optimizer, criterions, dl_x, dl_g, epoch):
        ...
        for n_i in range(n_iteration_per_epoch):
            ...
            try:
                Xs_u, ys_u = combine_train_iter.next()
    ...
    def model_accuracy(self, net):
        net.eval()
        correct_num = 0
        dlval = imagenet64.get_dataloader(train_datasets=imagenet64.get_test_dataset(), train_batch_size=self.config['test_batch_size'])
    ...
    def get_query_idx(self, net, unlabel_loader, n_select):
        ...
        with torch.no_grad():
            for (Xs, ys) in unlabel_loader:
                ...
                embedding_features.append(avg_feat.cpu())
