...
class PesudoSubset(Dataset):
    ...
    """
    def __init__(self, dataset, indices, pesudo_labels):
        self.dataset = dataset
        self.indices = indices
        self.pesudo_labels = pesudo_labels

    def __getitem__(self, idx):
        img, target = self.dataset[self.indices[idx]]
        return img, self.pesudo_labels[idx]

    def __len__(self):
        return len(self.pesudo_labels)
...
class MetricTrainer(BaseActiveTrainer):
    ...
    def start_loop(self, net, train_dataset, test_dataset, unlabel_dataset, n_select, n_iteration, index_loop=1, is_semi=False, is_semi_metric=False, pesudo_count=1000, semi_ind=None, is_mixup=True):
        ...
        total_train_ind = np.arange(22249)
        ...
        for i in range(n_iteration):
            ...
            dlval = mnist.get_dataloader(mnist.get_test_dataset(), self.config['test_batch_size'])
            ...
            ds_u = mnist.get_unlabel_dataset()
    ...
    def train_pesudo_data(self, model):
        ...
        ds_g = mnist.get_train_dataset()
        if self.pesudo_idx is None:
            ds_g = torch.utils.data.Subset(ds_g, self.label_ind)
        else:
            ds_p = PesudoSubset(ds_g, self.pesudo_idx, self.pesudo_label)
            ds_s = torch.utils.data.Subset(ds_g, self.label_ind)
            ds_g = torch.utils.data.ConcatDataset([ds_p, ds_s])
    ...
    def train_pesudo_each_epoch(self, model, optimizer, criterions, dl_x, dl_g, epoch):
        ...
        n_iteration_per_epoch = (len(dl_g.dataset) // self.config['train_batch_size'])+1
    ...
    def get_pesudo_idx(self, net, dl_u, thresh):
        ...
        pesudo_idx = np.where(metric_distance < 999999)[0]
